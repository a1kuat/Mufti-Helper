 ...
 */
export const Typewriter: React.FC<ITypewriter> = ({
    text,
    thinkingDelay = 4000,
    minTypingDelay = 50,
    maxTypingDelay = 100,
    ...
    const translateY = Number.isNaN(parsedHeight) ? 3 : parsedHeight * (translateYPercentage / 100);

    return (
        <span className='flex'>
            <span>
                <Markdown remarkPlugins={[remarkGfm]} skipHtml={false} className={textClassName}>
                    {displayText}
                </Markdown>
            </span>
            <span
                style={{
                    width: `${styles?.fontSize
                            ? Number(Number(parseInt(String(styles?.fontSize), 10) * (1 - 0.5)).toFixed(0)) + 'px'
                            : '10px'
                        }`,
                    height: `${styles?.fontSize
                            ? Number(Number(parseInt(String(styles?.fontSize), 10) * (1 - 0.1)).toFixed(0)) + 'px'
                            : '17px'
                        }`,
                    background: caretBackground,
                    transform: `translateY(${translateY}px)`,
                    display: notDisplayCaretAfterFinishes && finish ? 'none' : 'inline-block',
                    opacity: opacity,
                }}
            ></span>
        </span>
